...
def process_raw_data(file_path):
    ...
    #df = pd.read_excel(file_path, header=None, skiprows=1) #xlsx
    df = pd.read_csv(file_path, header=None, skiprows=1)
    
    # ── Column layout ──────────────────────────────────────────────
    # 0-8   : global features          (9)
    # 9-17  : per-segment ETAs         (9)   ← y source
    # 18    : separator                (1)
    # 19-54 : local features  9×4      (36)
    # 55    : start_segment  1-indexed (1)   ← NEW
    # 56    : total_trip_length        (1)   ← NEW
    # ──────────────────────────────────────────────────────────────
    ...
    # ── Start segment + total length ───────────────────────────────
    start_segs   = raw_data_np[:, 55].astype(int)       # 1-indexed, shape [N]
    total_length = raw_data_np[:, 56]                    # metres (or seconds), [N]
    
    # ── Segment mask  [N, 9]  True = active ───────────────────────
    seg_idx = np.arange(num_segment)[None, :]   # [1, 9]  0-indexed
    start_0 = (start_segs - 1)[:, None]         # [N, 1]  0-indexed
    mask_np = seg_idx >= start_0                # [N, 9]  bool
    
    # ── Global features: original 9 + one-hot(start) + ratio(length) ──
    start_onehot = np.eye(num_segment)[start_segs - 1]   # [N, 9]
    length_ratio = raw_data_np[:, 56].reshape(-1, 1)              # [N, 1], already in [0,1]

    x_global = torch.tensor(
        np.hstack([raw_data_np[:, 0:9], start_onehot, length_ratio]),
        dtype=torch.float32,
    )  # [N, 19]
    
    # ── Local features: zero out inactive segments ─────────────────
    raw_local  = raw_data_np[:, 9+num_segment+1 : 9+num_segment+1+(num_segment*4)]
    x_local_np = raw_local.reshape(-1, num_segment, 4)  # [N, 9, 4]
    #x_local_np[~mask_np] = 0.0                          # [N, 9, 4]
    x_local = torch.tensor(x_local_np, dtype=torch.float32)


    # ── Target y: sum only active segment ETAs ─────────────────────
    seg_etas = raw_data_np[:, 9 : 9+num_segment].copy() # [N, 9]
    seg_etas[~mask_np] = 0.0                    # zero out inactive before sum
    y_raw = seg_etas.sum(axis=1)                # [N]
    print(y_raw)
    ...
    # ── Mask tensor ────────────────────────────────────────────────
    segment_mask = torch.tensor(mask_np, dtype=torch.bool)  # [N, 9]
    
    print("Total rows loaded:", raw_data_np.shape[0])
    print("y_raw mean:", y_raw.mean())
    print("y_raw std:", y_raw.std())
    return x_global, x_local, y_scaled, segment_mask, scaler_y
...
class AttentionPooling(nn.Module):
    ...
    def forward(self, h_stack: torch.Tensor, mask: torch.Tensor = None, return_weights: bool = False):
        ...
        if mask is not None:
            # inactive segments → -inf so softmax gives them weight ≈ 0
...
class EncoderPhi(nn.Module):
    ...
    def forward(self, global_features, all_sections_data, segment_mask=None, return_attn: bool = False):
        ...
        if return_attn:
            h_pool, attn_weights = self.attn_pool(h_stack, mask=segment_mask, return_weights=True)
        else:
            h_pool = self.attn_pool(h_stack, mask=segment_mask)          # [B, hidden_dim]
...
class MatrixGNN_VAE(nn.Module):
    ...
    def forward(self, global_features, all_sections_data, segment_mask=None, return_attn=False, force_sample=False, kl_weight=1.0):
        # ── ENCODER → μ, log σ², log ν
        if return_attn:
            mu, logvar, attn_weights = self.encoder(global_features, all_sections_data, segment_mask=segment_mask, return_attn=True)
        else:
            mu, logvar = self.encoder(global_features, all_sections_data, segment_mask=segment_mask)
...
"""
def model_fn(x_global, x_local, y_true=None, segment_mask=None, total_size=None, kl_weight=1.0):
    pyro.module("bnn_model", bnn_model)
    locs, scales, kl = bnn_model(x_global, x_local, segment_mask=segment_mask)   # no kl_weight arg needed
...
def get_ll_kl(model_fn, guide_fn, x_g, x_l, y, segment_mask, total_size):
    with torch.no_grad():
        locs, scales, kl = bnn_model(x_g, x_l, segment_mask=segment_mask)   # 4-tuple now
        loc   = locs.squeeze(-1)
        scale = scales.squeeze(-1)
        #df    = dfs.squeeze(-1)
...
if __name__ == "__main__":
    ...
    def guide_fn(x_global, x_local, y_true=None, segment_mask=None, total_size=None, kl_weight=1.0):
        pass
    ...
    for epoch in range(epochs):
        ...
        if (epoch % 1 == 0 or epoch == epochs - 1):
            with torch.no_grad():
                ll, kl = get_ll_kl(model_fn, guide_fn, x_g_batch, x_l_batch, y_batch, mask_train, total_size=total_size)
    ...
    torch.save(bnn_model.state_dict(), "ghost_bus_vae_aggregate_all_length.pt")
    joblib.dump(scaler_y, "y_scaler_vae_aggregate_all_length.pkl")
