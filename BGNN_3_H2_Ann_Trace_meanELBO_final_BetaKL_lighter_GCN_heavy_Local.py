...
class LocalIsolationLayer(PyroModule):
    ...
    def forward(self, x_inputs):
        ...
        for i in range(self.num_segments):
            out1 = torch.nn.functional.silu(self.nets_1[i](x_inputs[i]))
            out2 = torch.nn.functional.silu(self.nets_2[i](out1))
            outputs.append(out2)
        return outputs
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        ramp_epochs = 250  # Spend 1500 epochs ramping up, 1000 epochs holding at 1.0
...
# ==========================================
    ...
    bnn_model.eval()
    base_guide.eval()
    pyro.get_param_store().save("ghost_bus_model_params.pt")
    print("\n--- Final Prediction Test ---")
    list_of_predict = []
    list_of_confidence = []
    list_of_actual = []
