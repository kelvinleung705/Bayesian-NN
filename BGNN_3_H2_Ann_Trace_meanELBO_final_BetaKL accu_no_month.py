...
def process_raw_data(file_path):
    ...
    # Extract the first 9 columns, then delete columns at index 4 and 5
    x_global_np = np.delete(raw_data_np[:, 0:9], [4, 5], axis=1)
    x_global = torch.tensor(x_global_np, dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    train_idx, val_idx = train_test_split(idx, test_size=0.001, random_state=42)
    ...
    #14
    bnn_model = MatrixGNN(num_sections=num_segment, global_dim=7, local_dim=4, hidden_dim=32, device=device).to(device)
    ...
    pyro.get_param_store().save("ghost_bus_model_cycle_0.1_2000_df10_KL_9_accu_Mar_Apr_no_month.pt")
    # Save the Y-Scaler to convert predictions back to seconds
    joblib.dump(scaler_y, "y_scaler_Mar_Apr_no_month.pkl")
