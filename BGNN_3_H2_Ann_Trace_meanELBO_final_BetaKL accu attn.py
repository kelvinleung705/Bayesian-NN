...
class NeighborMixingLayer(PyroModule):
    def __init__(self, input_dim, output_dim, num_segments, dropout_rate=0.2, device='cuda'):
        ...
        # ==========================================
        # 1. THE ATTENTION NETWORKS (The Dynamic Weights)
        # ==========================================
        # Instead of static w_self/w_right, we create a mini neural network
        # for EACH segment that calculates the attention scores.
        # Input: [Self_Features, Right_Features] (Size: input_dim * 2)
        # Output: [Score_Self, Score_Right] (Size: 2)
        self.att_nets = PyroModuleList([])
        for i in range(num_segments):
            att_net = PyroModule[nn.Linear](input_dim * 2, 2)
            
            # Priors for the Attention Mechanism
            att_net.weight = PyroSample(dist.Normal(zero, w_scale).expand([2, input_dim * 2]).to_event(2))
            
            # Bias initialization: We initialize it so 'Self' gets a slightly higher starting score
            # e.g., logit [1.0, 0.0] heavily favors Self initially before learning.
            att_bias_init = torch.tensor([1.0, 0.0], device=device)
            att_net.bias = PyroSample(dist.Normal(att_bias_init, b_scale).to_event(1))
            
            self.att_nets.append(att_net)
    ...
    def forward(self, prev_layer_outputs):
        outputs =[]
        for i in range(self.num_segments):
            # 1. Grab the feature vectors
            h_self = prev_layer_outputs[i] # Shape: [Batch, input_dim]
            
            if i < self.num_segments - 1:
                h_right = prev_layer_outputs[i+1]
            else:
                # The last segment has no right neighbor
                h_right = torch.zeros_like(h_self)
                
            # ==========================================
            # 2. CALCULATE DYNAMIC ATTENTION
            # ==========================================
            # Concatenate the features to form the context for the attention network
            # Context shape: [Batch, input_dim * 2]
            context = torch.cat([h_self, h_right], dim=1)
            
            # Pass the context through the attention network to get RAW scores
            # Shape: [Batch, 2]
            raw_scores = self.att_nets[i](context)
            
            # Apply Softplus to convert raw scores into percentages that sum to 1.0
            # e.g., raw [2.5, -0.5] -> softplus -> [0.95, 0.05] (95% focus on Self)
            #attention_weights = torch.nn.functional.softplus(raw_scores)
            attention_weights = torch.sigmoid(raw_scores) * 2.0
            
            # Extract the weights (and ensure shapes match for multiplication)
            alpha_self = attention_weights[:, 0].unsqueeze(1)  # Shape: [Batch, 1]
            #print()  # Debug: Print first 5 attention weights for Self
            alpha_right = attention_weights[:, 1].unsqueeze(1) # Shape: [Batch, 1]
            #print(alpha_self[0], alpha_right[0])  # Debug: Print first 5 attention weights for Right

            # 3. APPLY ATTENTION WEIGHTS TO FEATURES
            # The features are multiplied by their dynamic percentage
            self_feat = h_self * alpha_self
            right_feat = h_right * alpha_right
...
if __name__ == "__main__":
    ...
    def guide_fn(x_global, x_local, y_true=None, total_size=None, kl_weight=1):
        with pyro.poutine.scale(scale=kl_weight):
    ...
    pyro.get_param_store().save("ghost_bus_model_cycle_0.1_2000_df10_KL_9_accu_attn.pt")
    # Save the Y-Scaler to convert predictions back to seconds
    joblib.dump(scaler_y, "y_scaler_1.pkl")
...
# ==========================================
    ...
    bnn_model.eval()
    base_guide.eval()
    pyro.get_param_store().save("ghost_bus_model_params_4_8_test.pt")
    print("\n--- Final Prediction Test ---")
    list_of_predict = []
    list_of_confidence = []
    list_of_actual = []
