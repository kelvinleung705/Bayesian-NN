...
class LocalIsolationLayer(PyroModule):
    ...
    def __init__(self, input_dim, output_dim, num_segments, device = 'cuda'):
        ...
        self.nets1 = PyroModuleList([])
        self.nets2 = PyroModuleList([])
        
        for i in range(num_segments):
            # Simple Linear transformation for this specific segment
            net1 = PyroModule[nn.Linear](input_dim, output_dim)
            zero = torch.tensor(0., device=device)
            point_one = torch.tensor(1, device=device)
            df = torch.tensor(15., device=device)
            net1.weight = PyroSample(dist.StudentT(df, zero, point_one).expand([output_dim, input_dim]).to_event(2))
            net1.bias = PyroSample(dist.StudentT(df, zero, point_one).expand([output_dim]).to_event(1))
            self.nets1.append(net1)
            net2 = PyroModule[nn.Linear](output_dim, output_dim)
            zero = torch.tensor(0., device=device)
            point_one = torch.tensor(1, device=device)
            df = torch.tensor(15., device=device)
            net2.weight = PyroSample(dist.StudentT(df, zero, point_one).expand([output_dim, output_dim]).to_event(2))
            net2.bias = PyroSample(dist.StudentT(df, zero, point_one).expand([output_dim]).to_event(1))
            self.nets2.append(net2)

    def forward(self, x_inputs):
        # x_inputs is a list of tensors, one per segment
        outputs = []
        for i in range(self.num_segments):
            out = torch.nn.functional.silu(self.nets1[i](x_inputs[i]))
            out = torch.nn.functional.silu(self.nets2[i](out))
            outputs.append(out)
...
if __name__ == "__main__":
    ...
    train_idx, val_idx = train_test_split(idx, test_size=0.05, random_state=42)
    ...
    for epoch in range(epochs):
        ...
        max_beta = 0.12  # Max KL weight (equivalent to saying "Data is 10x more important than Prior")
    ...
    pyro.get_param_store().save("ghost_bus_model_cycle_0.1_2000_df10_KL_9_accu_moreEmbed.pt")
    # Save the Y-Scaler to convert predictions back to seconds
    joblib.dump(scaler_y, "y_scaler_moreEmbed.pkl")
