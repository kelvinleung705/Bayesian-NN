...
class MatrixGNN(PyroModule):
    def __init__(self, num_sections=3, global_dim=12, local_dim=4, hidden_dim=8, device = 'cuda', pnt_1 = None):
        ...
        for i in range(self.num_sections):
            ...
            # Mmy 2
            zero = torch.tensor(0., device=device)
            loc_std = torch.tensor(2.0, device=device) #1.0
            loc_bias_mu = torch.tensor(0., device=device) 
            loc_bias_std = torch.tensor(2., device=device) #1(16-32)

            scale_std = torch.tensor(3., device=device) 
...
if __name__ == "__main__":
    ...
    pyro.get_param_store().save("ghost_bus_no_cascade.pt")
    # Save the Y-Scaler to convert predictions back to seconds
    joblib.dump(scaler_y, "y_scaler_no_cascade.pkl")
