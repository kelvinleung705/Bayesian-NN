...
def process_raw_data(file_path):
    ...
    print(scaler_y.mean_, scaler_y.scale_)  # Check the mean and scale used for normalization
...
class BayesianMLPHead(PyroModule):
    def __init__(self, in_dim, hidden_dim, out_dim, device='cuda', 
                 weight_loc=0., weight_scale=1.0, bias_loc=0., bias_scale=0.1):
        super().__init__()
        # 第一層 (隱藏層)
        ...
        self.layer1.bias = PyroSample(dist.Normal(torch.tensor(bias_loc, device=device), 
...
if __name__ == "__main__":
    ...
    pyro.get_param_store().save("ghost_bus_no_cascade_dedicated_mlp.pt")
    # Save the Y-Scaler to convert predictions back to seconds
    joblib.dump(scaler_y, "y_scaler_no_cascade_dedicated_mlp.pkl")
