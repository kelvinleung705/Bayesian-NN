...
class MatrixGNN(PyroModule):
    def __init__(self, num_sections=3, global_dim=12, local_dim=4, hidden_dim=8, device = 'cuda', pnt_1 = None):
        ...
        iso_layer_num = 3
        self.iso_layer = PyroModuleList([PyroModuleList([]) for _ in range(iso_layer_num)])
        for layer_idx in range(iso_layer_num):
            for i in range(self.num_sections):
            # Isolation Layers for each section
                zero = torch.tensor(0., device=device)
                loc_std = torch.tensor(1.0, device=device)
                loc_bias_mu = torch.tensor(0., device=device)
                loc_bias_std = torch.tensor(1., device=device)
                iso_loc = PyroModule[nn.Linear](hidden_dim, hidden_dim)
                iso_loc.weight = PyroSample(dist.Normal(zero, loc_std).expand([hidden_dim, hidden_dim]).to_event(2))
                iso_loc.bias = PyroSample(dist.Normal(loc_bias_mu, loc_bias_std).expand([hidden_dim]).to_event(1))
                self.iso_layer[layer_idx].append(iso_loc)
        self.dropout_3 = PyroModule[nn.Dropout](p=0.2)
    ...
    def forward(self, global_features, all_sections_data):
        ...
        for current_section in range(self.num_sections):
            ...
            # Pass ONLY this feature through the Isolation layers
            for sub_layer_list in self.iso_layer:
                final_feat = sub_layer_list[current_section](final_feat)
                final_feat = torch.nn.functional.silu(final_feat) # Activation
                final_feat = self.dropout_3(final_feat)  
...
if __name__ == "__main__":
    ...
    saved_params_path = "ghost_bus_model_cycle_1_2000_df10_KL_9_accu_mlp.pt" # Replace with exact saved params file name
    
    saved_scaler_path = "y_scaler_2.pkl"              # Replace with exact saved scaler file name
