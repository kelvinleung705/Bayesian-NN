...
def export_for_netron(bnn_model, guide_fn, x_global_val, x_local_val,
                      device, export_format="onnx"):
    """
    export_format: "onnx"  → matrix_gnn.onnx  (best Netron support)
                   "torchscript" → matrix_gnn.pt
    """
    bnn_model.eval()

    dummy_g = x_global_val[0:2].to(device)   # batch of 2 so shapes are visible
    dummy_l = x_local_val[0:2].to(device)

    print("\n--- Capturing frozen guide trace ---")
    with torch.no_grad():
        # One deterministic sample from the trained posterior
        guide_trace = pyro.poutine.trace(guide_fn).get_trace(dummy_g, dummy_l)

    wrapper = NetronExportWrapper(bnn_model, guide_trace).to(device)
    wrapper.eval()

    if export_format == "onnx":
        out_path = "matrix_gnn.onnx"
        with torch.no_grad():
            torch.onnx.export(
                wrapper,
                (dummy_g, dummy_l),
                out_path,
                input_names  = ["global_features", "local_features"],
                output_names = ["pred_loc", "pred_scale", "pred_df"],
                dynamic_axes = {
                    "global_features": {0: "batch_size"},
                    "local_features":  {0: "batch_size"},
                    "pred_loc":        {0: "batch_size"},
                    "pred_scale":      {0: "batch_size"},
                    "pred_df":         {0: "batch_size"},
                },
                opset_version=17,
            )
        ...
        out_path = "matrix_gnn.pt"
        with torch.no_grad():
            # trace() records the exact tensor ops from one forward pass
            traced = torch.jit.trace(wrapper, (dummy_g, dummy_l),
                                     strict=False)   # strict=False handles Pyro list returns
        traced.save(out_path)
        print(f"✅ TorchScript exported → {out_path}")

    else:
        raise ValueError("export_format must be 'onnx' or 'torchscript'")
...
# --- Call it right after Section 2 (after guide_fn dummy trace) ---
    export_for_netron(
        bnn_model, guide_fn,
        x_global_val, x_local_val,
        device,
        export_format="onnx"           # swap to "torchscript" if ONNX fails
    )
