...
class MatrixGNN(PyroModule):
    def __init__(self, num_sections=3, global_dim=12, local_dim=4, hidden_dim=8, device = 'cuda', pnt_1 = None):
        super().__init__()
        self.time_norm = nn.BatchNorm1d(1, affine=False) # affine=False means no learned weights, just raw normalization
...
if __name__ == "__main__":
    ...
    pyro.get_param_store().save("ghost_bus_model_cycle_0.1_2000_df10_KL_9_accu_flat.pt")
    # Save the Y-Scaler to convert predictions back to seconds
    joblib.dump(scaler_y, "y_scaler_flat.pkl")
