...
class MatrixGNN(PyroModule):
    def __init__(self, num_sections=3, global_dim=12, local_dim=4, hidden_dim=8, device = 'cuda', pnt_1 = None):
        ...
        loc_self = torch.tensor(1., device=device)
        scale = torch.tensor(1.0, device=device)
        for _ in range(3):
            net_1 = PyroModule[nn.Linear](hidden_dim, hidden_dim)
            net_1.weight = PyroSample(dist.Normal(loc_self, scale).expand([hidden_dim, hidden_dim]).to_event(2))
            net_1.bias = PyroSample(dist.Normal(loc_self, scale).expand([hidden_dim]).to_event(1))
            self.prop_layers_2.append(net_1)
    ...
    def forward(self, global_features, all_sections_data):
        ...
        for current_section in range(self.num_sections):
            ...
            for layer in self.prop_layers_2:
                h_current = layer(h_current)
