...
class NeighborMixingLayer(PyroModule):
    ...
    def forward(self, prev_layer_outputs):
        ...
        for i in range(self.num_segments):
            # 1. Scaling the self feature
            ws = torch.nn.functional.softplus(self.w_self[i])
            self_feat = prev_layer_outputs[i] * ws
            
            # 2. Layer 1
            # No torch.cat needed because there are no neighbors
            out = self.nets_1[i](self_feat) 
...
if __name__ == "__main__":
    ...
    pyro.get_param_store().save("ghost_bus_model_cycle_0.1_2000_df10_KL_non_gnn.pt")
