...
class MatrixGNN(PyroModule):
    ...
    def forward(self, global_features, all_sections_data):
        ...
        all_feats = {i: [] for i in range(self.num_sections)}
        ...
        for current_section in range(self.num_sections):
            ...
            all_feats[current_section].append(final_feat.detach().cpu())  # ← ADD THIS LINE
        ...
        for s in range(self.num_sections):
            all_feats[s] = torch.cat(all_feats[s], dim=0)  # (N, hidden_dim)

    # ✅ INSERT 4 — Update return to include all_feats
        return all_locs, all_scales, all_dfs, all_feats
...
def model_fn(x_global, x_local, y_true=None, total_size=None, kl_weight=1.0):
    with pyro.poutine.scale(scale=kl_weight):
        locs, scales, dfs, feats = bnn_model(x_global, x_local)
...
if __name__ == "__main__":
    ...
    saved_params_path = "ghost_bus_model_cycle_0.9_clamp_6_6_2133.pt"
...
plt.savefig('loc_vs_scale_median_centred_final.png', dpi=150, bbox_inches='tight')
plt.show()
print("Saved → loc_vs_scale_median_centred_final.png")
