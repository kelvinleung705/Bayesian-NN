...
class Annealer:
    ...
    def __init__(self, total_steps, shape='linear', baseline=0.0, maximum = 1.0, cyclical=False, disable=False):
        """
        Parameters:
            ...
            max (float): Maximum value for the annealing function. Default is 1.0.
        ...
        if not 0 <= float(maximum) <= 1:
            raise ValueError("Maximum must be a float between 0 and 1.")
    ...
    def _slope(self):
        if self.shape == 'linear':
            y = min((self.current_step / (self.total_steps/2)), 1.0) * self.maximum
