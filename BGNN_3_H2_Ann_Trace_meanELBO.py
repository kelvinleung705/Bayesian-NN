...
if __name__ == "__main__":
    ...
    #annealing_constructor = Annealer(total_steps=500, shape='linear', baseline=0.0, maximum = 0.05, cyclical=True, disable=False)
    annealing_constructor = Annealer(total_steps=, shape='linear', baseline=0.0, maximum = 1, cyclical=True, disable=False)
    for epoch in range(epochs):
        ...
        current_kl_weight = annealing_constructor._slope() + 0.00001
        ...
        with torch.no_grad():
            # We use the guide to get the median (most likely) values for all weights
            # This returns a dictionary of all sampled sites
            latent_means = guide.quantiles([0.5]) 

            # The naming convention in Pyro for your setup:
            # "embedding_layer.nets.0.weight"
            # "embedding_layer.nets.8.weight"

            for i in [0, num_segment - 1]:
                weight_name = f"embedding_layer.nets.{i}.weight"
                bias_name = f"embedding_layer.nets.{i}.bias"

                if weight_name in latent_means:
                    w = latent_means[weight_name].squeeze()
                    b = latent_means[bias_name].squeeze()

                    print(f"Segment {i} :")
                    print(f"   Weight Mean Absolute: {w.abs().mean().item():.6f}")
                    print(f"   Weight Max/Min: {w.max().item():.4f} / {w.min().item():.4f}")
                    print(f"   Bias Value: {b.mean().item():.4f}")
