...
class NeighborMixingLayer(PyroModule):
    ...
    def __init__(self, input_dim, output_dim, num_segments, dropout_rate=0.2):
        ...
        for i in range(num_segments):
            ...
            net_1.weight = PyroSample(dist.Normal(0., 0.2).expand([output_dim, net_input_dim]).to_event(2))
            
            net_1.bias = PyroSample(dist.Normal(0., 0.1).expand([output_dim]).to_event(1))
            self.nets_1.append(net_1)
    
        self.dropout1 = PyroModule[nn.Dropout](p=dropout_rate)
        ...
        self.dropout2 = PyroModule[nn.Dropout](p=dropout_rate)
...
if __name__ == "__main__":
    ...
    bnn_model = MatrixGNN(num_sections=num_segment, global_dim=14, local_dim=4, hidden_dim=18)
