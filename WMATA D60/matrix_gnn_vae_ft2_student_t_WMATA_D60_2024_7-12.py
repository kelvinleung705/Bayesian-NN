...
class EncoderPhi(nn.Module):
    ...
    def forward(self, global_features, all_sections_data, return_attn: bool = False):
        ...
        # FIX: Clamp logvar to prevent exp() overflow and underflow
...
class DecoderTheta(nn.Module):
    ...
    def forward(self, z, x_global):
        ...
        # Tighter clamp on log-scale (-6.0 to 4.0 keeps scale between ~0.002 and ~54.0)
        b_t = torch.exp(0.5 * torch.clamp(b_e, min=-6.0, max=4.0)) + 1e-3
        
        # Constrain df between 2.5 and 30.0 to prevent gradient explosions
        df_e = self.heads_df(h_b)
        nu = torch.clamp(torch.nn.functional.softplus(df_e) + 2.5, min=2.5, max=30.0)
...
def guide_fn(x_global, x_local, y_true=None, total_size=None, kl_weight=1.0):
    ...
    with pyro.plate("data", size=total_size, subsample_size=x_global.shape[0]):
        ...
        logvar = torch.clamp(logvar, min=-8.0, max=8.0) # Safeguard
        sigma = torch.exp(0.5 * logvar) + 1e-4          # Add small eps
...
if __name__ == "__main__":
    ...
    # ── 1. Pure PyTorch AdamW with gradient clipping inside .step() ──
    class TorchClippedAdamW(torch.optim.AdamW):
        def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                     weight_decay=0.01, amsgrad=False, max_grad_norm=5.0):
            super().__init__(params, lr=lr, betas=betas, eps=eps,
                             weight_decay=weight_decay, amsgrad=amsgrad)
            self.max_grad_norm = max_grad_norm

        def step(self, closure=None):
            # Clip gradients for all parameters in all groups
            for group in self.param_groups:
                torch.nn.utils.clip_grad_norm_(group["params"], self.max_grad_norm)
            return super().step(closure)

    # ── 2. Configure PyroLRScheduler with the custom PyTorch optimizer ──
    CYCLE_LENGTH = 1000

    optimizer_args = {
        "optimizer": TorchClippedAdamW,
        "optim_args": {
            "lr": 0.0005,
            "weight_decay": 0.0001,
            "max_grad_norm": 5.0,  # Limits gradient norm to 5.0
        },
    }
    ...
    torch.save(bnn_model.state_dict(), "2024_Jul_Dec_vae_ft_student_t_0.7_WMATA_D60.pt")
    joblib.dump(scaler_y, "y_scaler_vae_student_t_2024_Jul_Dec_WMATA_D60.pkl")
