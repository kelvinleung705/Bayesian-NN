...
class MatrixGNN(PyroModule):
    def __init__(self, num_sections=3, global_dim=12, local_dim=4, hidden_dim=8, device = 'cuda', pnt_1 = None):
        ...
        self.prop_layers_2 = PyroModuleList([])
        zero = torch.tensor(0., device=device)
        w_scale = torch.tensor(1.0, device=device)
        b_scale = torch.tensor(0.1, device=device)
        for _ in range(3):
            # Inside each layer, we want 'num_sections' independent networks
            section_specific_nets = PyroModuleList([])
            
            for _ in range(self.num_sections):
                net = PyroModule[nn.Linear](hidden_dim, hidden_dim)
                
                # Assign Bayesian priors to this specific network
                net.weight = PyroSample(dist.Normal(zero, w_scale).expand([hidden_dim, hidden_dim]).to_event(2))
                net.bias = PyroSample(dist.Normal(zero, b_scale).expand([hidden_dim]).to_event(1))
                
                section_specific_nets.append(net)
                
            # Append the list of 9 networks to the main layer list
            self.prop_layers_2.append(section_specific_nets)
    ...
    def forward(self, global_features, all_sections_data):
        ...
        for current_section in range(self.num_sections):
            ...
            for layer_group in self.prop_layers_2:
                # layer_group has 9 networks, h_list has 9 tensors.
                # zip(layer_group, h_list) pairs Network 0 with Tensor 0, Network 1 with Tensor 1, etc.
                h_current = [net(h) for net, h in zip(layer_group, h_current)]
...
if __name__ == "__main__":
    ...
    pyro.get_param_store().save("ghost_bus_model_cycle_1_2000_df10_KL_9_accu_2.pt")
...
# ==========================================
    ...
    bnn_model.eval()
    base_guide.eval()
    pyro.get_param_store().save("ghost_bus_model_params_4/8_test.pt")
    print("\n--- Final Prediction Test ---")
    list_of_predict = []
    list_of_confidence = []
    list_of_actual = []
