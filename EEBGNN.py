...
class NeighborMixingLayer(PyroModule):
    ...
    def __init__(self, input_dim, output_dim, num_segments, dropout_rate=0.2):
        ...
        for i in range(num_segments):
            ...
            net.weight = PyroSample(dist.Normal(0., he_std).expand([output_dim, net_input_dim]).to_event(2))
            net.bias = PyroSample(dist.Normal(0., 0.1).expand([output_dim]).to_event(1))
            self.nets.append(net)
...
class MatrixGNN(PyroModule):
    def __init__(self, num_sections=3, global_dim=12, local_dim=4, hidden_dim=8):
        ...
        for i in range(self.num_layer):
            ...
            gate.weight = PyroSample(dist.Normal(0., 0.2).expand([1, gate_input_dim]).to_event(2))
            #-2.0
            gate.bias = PyroSample(dist.Normal(0.0, 0.2).expand([1]).to_event(1)) #0.2
            self.exit_gates.append(gate)
        
    
    def predict_step(self, h_current):
    ...
    def forward(self, global_features, all_sections_data):
        ...
        preds, gates = self.predict_step(h_current)
        ...
        for layer in self.prop_layers:
            ...
            predictions, gates = self.predict_step(h_current)
